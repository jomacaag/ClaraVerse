/**
 * Clara Agent API Service
 * 
 * Specialized API service for agent execution with custom provider support.
 * This is a copy of ClaraApiService with modifications to handle custom providers
 * used specifically in Agent Studio workflows.
 */
...
`.trim();
  ...
  public async sendChatMessage(
    ...
    try {
      ...
      // Extract model ID from provider prefix if present (but NOT for custom providers)
      if (config.provider !== 'custom') {
        modelId = claraModelService.extractModelId(modelId);
        console.log(`🔧 Extracted model ID: ${modelId} (from provider: ${config.provider})`);
      } else {
        console.log(`🔧 Using exact model ID for custom provider: ${modelId}`);
      }
...
**If you're using Clara Core:** The model may not be loaded correctly. Please go to Settings and apply the Balanced configuration to resolve this issue.`;
  ...
  /**
   * Ensure we're using the correct provider - WITH CUSTOM PROVIDER SUPPORT
   */
  private async ensureCorrectProvider(config: ClaraAIConfig, _onContentChunk?: (content: string) => void): Promise<void> {
    ...
    if (config.provider && (!currentProvider || currentProvider.id !== config.provider)) {
      ...
      try {
        // Handle custom provider - THIS IS THE KEY DIFFERENCE
        if (config.provider === 'custom' && config.customProvider) {
          ...
          // Create a temporary provider object for the custom provider
          const customProvider: ClaraProvider = {
            id: 'custom',
          ...
          // Update the client to use the custom provider
          claraProviderService.updateProvider(customProvider);
          console.log(`🚀 Switched to custom provider: ${customProvider.baseUrl}`);
          
          return;
        }
        
        // Handle regular providers from database
  ...
  public async preloadModel(config: ClaraAIConfig, conversationHistory?: ClaraMessage[]): Promise<void> {
    ...
    if (config.provider !== 'custom') {
      modelId = claraModelService.extractModelId(modelId);
      console.log(`🔧 [PRELOAD] Extracted model ID: ${modelId} (from provider: ${config.provider})`);
    } else {
      console.log(`🔧 [PRELOAD] Using exact model ID for custom provider: ${modelId}`);
    }
...
// Export singleton instance for agent execution
export const claraAgentApiService = new ClaraAgentApiService();
